...
object ChromeExtensionInstaller {
    ...
    fun parseCrxPayload(extensionId: String, zipBytes: ByteArray): ParsedExtensionPayload {
        ...
        return ParsedExtensionPayload(
            ...
            popupHtml = popupPath?.let { buildPopupHtml(it, files) }
        )
    }

    private fun buildPopupHtml(path: String, files: Map<String, String>): String? {
        val raw = files[path] ?: return null
        return inlinePopupAssets(raw, files)
    }

    private fun inlinePopupAssets(html: String, files: Map<String, String>): String {
        var result = html
        val scriptRegex = Regex("""<script[^>]*src=["']([^"']+)["'][^>]*>\s*</script>""")
        scriptRegex.findAll(html).forEach { match ->
            val src = match.groupValues[1]
            val script = files[src] ?: return@forEach
            val replacement = "<script>\n$script\n</script>"
            result = result.replace(match.value, replacement)
        }

        val styleRegex = Regex("""<link[^>]*rel=["']stylesheet["'][^>]*href=["']([^"']+)["'][^>]*>""")
        styleRegex.findAll(result).forEach { match ->
            val href = match.groupValues[1]
            val css = files[href] ?: return@forEach
